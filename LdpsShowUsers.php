...
if (! class_exists('LdpsShowUsers')) :
    ...
    class LdpsShowUsers
    {
        ...
        public function __construct()
        {
            ...
            add_action('admin_menu', [$this, 'ldpsLinkVirtualPageUrl']);

            // Detect the slug and hijack it if it matches our desired slug
            add_filter('template_include', [$this, 'ldpsDoVirtualPageTemplate'], 99);

            // Add to sidebar our settings link
            add_action('admin_menu', [$this, 'ldpsLinkSettingsSidebar']);

            // Add settings to plugin page
            add_filter('plugin_action_links_ldps-show-users/ldps-show-users.php', [$this, 'ldpsLinkSettingsPluginPage']);

            add_action('wp_print_styles', [$this, 'ldpsControlStyles'], 100); // We use this hook because the theme's styles are not loaded yet on wp_enqueue_scripts so we have to remove the theme's files AFTER wordpress enqueues them

            // Initialize options
            add_action('admin_init', [$this, 'ldpsShowUsersOptionsInit']);

            // Create settings page
            add_action('admin_menu', [$this, 'ldpsShowUsersOptionsAddPage']);
        }

        /**
         * Add direct access to our virtual page on the topmost level of our admin menu
         */
        public function ldpsLinkVirtualPageUrl()
        {
         ...
         */
        public function ldpsDoVirtualPageTemplate(string $template) : ?string
        {
         ...
         */
        public function ldpsLinkSettingsSidebar()
        {
         ...
         */
        public function ldpsLinkSettingsPluginPage(array $links) : array
        {
         ...
         */
        public function ldpsControlStyles()
        {
            global $wp_styles;
        ...
        public function ldpsShowUsersOptionsInit()
        {
            register_setting('ldps_show_users_options', 'ldps_show_users', 'ldpsShowUsersOptionsValidate');
        }

        // Add menu page
        public function ldpsShowUsersOptionsAddPage()
        {
            add_options_page('Show Users Options', 'Show Users Options', 'manage_options', 'ldps_show_users_options', [$this, 'ldpsShowUsersOptionsDoPage']);
        }

        // Draw the menu page itself
        public function ldpsShowUsersOptionsDoPage()
        {
         ...
         * @return array $input
         */
        public function ldpsShowUsersOptionsValidate(array $input) : array
        {
